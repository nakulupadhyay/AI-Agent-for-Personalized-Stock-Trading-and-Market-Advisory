    ...
    <div className="space-y-6 sm:space-y-8 animate-fade-in w-full max-w-7xl mx-auto will-change-transform">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="text-slate-500 dark:text-slate-400 mt-1">
      ...
      {initialLoading && !stats ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card bg-gradient-to-br from-slate-100 to-slate-50 dark:from-dark-200/80 dark:to-dark-100/50 border border-slate-200 dark:border-t-primary-500/30">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">{statCards[0].label}</p>
              <h2 className="text-3xl font-black text-slate-900 dark:text-white">{statCards[0].value}</h2>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 }} className="card bg-slate-50 dark:bg-dark-200/50">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="card bg-slate-50 dark:bg-dark-200/50">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">{statCards[2].label}</p>
              <h2 className="text-3xl font-black text-slate-700 dark:text-slate-200">{statCards[2].value}</h2>
            </motion.div>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card flex-1 flex flex-col min-h-[340px] will-change-contents">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="px-3 py-1 bg-white dark:bg-dark-100 rounded-lg text-xs font-medium text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800">Live</div>
                  </div>
                </div>
                <div className="flex-1 w-full relative min-h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={MOCK_CHART} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                      ...
                      />
                      <Area type="monotone" dataKey="value" stroke="#6C63FF" strokeWidth={3} fill="url(#colorPrimary)" activeDot={{ r: 6, fill: '#6C63FF' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-primary-100/50 to-slate-50 dark:from-primary-900/40 dark:to-dark-200 border border-primary-500/20 p-6 flex flex-col sm:flex-row items-center gap-5 will-change-contents"
              >
                ...
                <div className="flex-1 text-center sm:text-left relative z-10">
                  ...
                  <h4 className="text-lg text-slate-900 dark:text-white font-medium">AI suggests accumulating <strong className="text-primary-400 font-black">INFY</strong></h4>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">High confidence score (87%) based on recent technical breakouts and sector momentum.</p>
                </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Top Movers</h3>
                ...
                {stocks.map((stock, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group relative bg-slate-50 dark:bg-dark-200/40 backdrop-blur-md border border-slate-200 dark:border-slate-800 hover:border-slate-600 rounded-xl p-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl cursor-pointer will-change-transform"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="font-bold text-slate-900 dark:text-white text-base group-hover:text-primary-300 transition-colors">{stock.symbol}</h4>
                      ...
                      <div className="flex justify-between items-end mt-2">
                        <p className="text-lg font-black text-slate-900 dark:text-white transition-opacity duration-300">{formatCurrency(stock.currentPrice)}</p>
