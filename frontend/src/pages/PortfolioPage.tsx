...
export default function PortfolioPage() {
  ...
  const loadData = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      setPortfolio(data.data ?? data ?? DEMO);
    } catch {
  ...
  const animValue = useAnimatedNumber(portfolio?.totalValue || 0);
  const animInvested = useAnimatedNumber(portfolio?.totalInvested || 0);
  const animPnl = useAnimatedNumber(portfolio?.totalGainLoss || 0);
  ...
  const processedHoldings = useMemo(() => {
    ...
    return portfolio.holdings
      .filter((h: any) => h.symbol.toLowerCase().includes(searchQuery.toLowerCase()) || h.name.toLowerCase().includes(searchQuery.toLowerCase()))
      .sort((a: any, b: any) => {
  ...
  const pieData = useMemo(() => {
    return portfolio?.holdings.map((h: any) => ({
      name: h.symbol, value: h.currentValue, allocation: h.allocation,
    })) ?? [];
  }, [portfolio]);
  ...
  const topCards = useMemo(() => {
    ...
    return [
      { label: 'Total Portfolio Value', value: formatCurrency(animValue), bg: 'bg-white/5 border-white/10', icon: Wallet, color: '#6C63FF', chart: generateSparkline() },
      { label: 'Total Invested Amount', value: formatCurrency(animInvested), bg: 'bg-white/5 border-white/10', icon: Briefcase, color: '#94a3b8', chart: generateSparkline() },
      { label: 'Net Profit & Loss',
        value: `${isProfitable ? '+' : ''}${formatCurrency(Math.abs(animPnl))}`,
        sub: formatChange(portfolio.totalGainLossPercent) + '% Returns',
        isPnl: true,
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-[#020617] via-[#0f172a] to-[#020617] -m-4 sm:-m-6 lg:-m-8 p-4 sm:p-6 lg:p-8">
      <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-10">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
          <div>
            <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-4xl font-black text-white tracking-tight">
              My Portfolio
            </motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="text-slate-400 mt-2 text-sm font-medium">
                ...
                <motion.div key={c.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="flex justify-between items-start relative z-10">
                    <div>
                      ...
                      <h2 className={`mt-2 text-3xl font-black tracking-tight ${c.isPnl ? (animPnl >= 0 ? 'text-bull' : 'text-bear') : 'text-white'}`}>
                        {c.value}
                      </h2>
                      {c.sub && <p className="mt-1 text-sm font-bold opacity-90" style={{ color: c.color }}>{c.sub}</p>}
                    </div>
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/5 shadow-inner" style={{ color: c.color }}>
                      <c.icon size={24} />
                    </div>
                  </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
              ...
              <div className="lg:col-span-1 flex flex-col gap-6">
                ...
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                            className="rounded-2xl border border-white/5 bg-white/5 backdrop-blur-xl p-6 shadow-2xl flex flex-col hover:border-white/10 transition-colors will-change-transform">
                  <div className="flex items-center justify-between mb-2">
                    <h2 className="font-black text-lg text-white">Asset Allocation</h2>
                    <div className="p-2 bg-white/5 rounded-xl border border-white/5"><Activity size={16} className="text-primary-500" /></div>
                  </div>
                  ...
                  {pieData.length > 0 ? (
                    <div className="flex-1 min-h-[250px] relative mt-4">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mb-4">
                        ...
                        <span className="text-white text-3xl font-black">{animPieLength.toFixed(0)}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-500 border border-dashed border-white/10 rounded-2xl min-h-[200px] mt-4">
                      ...
                      <p className="font-medium text-sm text-slate-400">No holdings to analyze</p>
                    </div>
                  )}
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                            className="rounded-2xl border border-primary-500/30 bg-gradient-to-b from-primary-900/20 to-transparent backdrop-blur-xl p-6 shadow-2xl relative overflow-hidden group will-change-transform">
                   ...
                   <div className="flex items-center gap-3 mb-4 relative z-10">
                     ...
                     <h2 className="font-black text-lg text-white">AI Insights</h2>
                   </div>
                   
                   <div className="space-y-4 relative z-10">
                     <div className="bg-dark-100/40 rounded-xl p-4 border border-white/5 hover:border-bull/30 transition-colors">
                       ...
                       <p className="text-xs text-slate-400 font-medium leading-relaxed">AI suggests accumulating TCS based on an imminent technical breakout combined with solid sector momentum.</p>
                     </div>
                     <div className="bg-dark-100/40 rounded-xl p-4 border border-white/5 hover:border-bear/30 transition-colors">
                       ...
                       <p className="text-xs text-slate-400 font-medium leading-relaxed">Avoid increasing positions in ICICI Bank short-term due to underlying MACD divergence detected by our core modeling.</p>
                     </div>
                   </div>
                </motion.div>
              </div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
                          className="lg:col-span-2 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden max-h-[850px] will-change-transform">
                
                <div className="p-6 border-b border-white/5 bg-dark-100/20 sm:flex justify-between items-center space-y-4 sm:space-y-0">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 font-medium">Manage and review your live holdings</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative flex items-center w-full sm:w-auto">
                      ...
                      <input 
                        ...
                        className="pl-9 pr-4 py-2 bg-dark-200/50 border border-white/10 rounded-xl text-sm text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none w-full shadow-inner transition-all hover:bg-dark-200/80"
                      />
                    </div>
                    <button className="p-2.5 bg-dark-200/50 border border-white/10 rounded-xl text-slate-400 hover:text-white hover:border-white/30 transition-all">
                      <Filter size={16} />
                    </button>
                  </div>
                </div>
                ...
                <div className="overflow-y-auto overflow-x-auto flex-1 custom-scrollbar">
                  <table className="w-full text-sm text-left border-collapse">
                    <thead className="sticky top-0 z-20 bg-[#0f172a]/95 backdrop-blur-md shadow-sm border-b border-white/5">
                      <tr>
                        {[
                          ...
                          <th key={col.key} onClick={() => handleSort(col.key)} className="px-6 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors select-none">
                              ...
                              >
                                ...
                                <td className="px-6 py-5">
                                  <div className="flex items-center gap-4">
                                    ...
                                    <div className="min-w-0">
                                      <p className="font-black text-white text-base truncate group-hover:text-primary-300 transition-colors">{h.symbol}</p>
                                      <p className="text-[11px] font-medium text-slate-500 truncate max-w-[120px]">{h.name}</p>
                                    </div>
                                  </div>
                                </td>

                                <td className="px-6 py-5">
                                  <p className="font-bold text-white text-base">{h.quantity}</p>
                                  <p className="text-[10px] uppercase font-bold text-slate-600 tracking-wider">Shares</p>
                                </td>

                                <td className="px-6 py-5">
                                  <p className="font-bold text-slate-300">{formatCurrency(h.avgPrice)}</p>
                                </td>

                                <td className="px-6 py-5">
                                  <p className="font-black text-white">{formatCurrency(h.currentPrice)}</p>
                                  <p className={`text-[10px] font-bold mt-0.5 tracking-wide ${(h.currentPrice % 2 === 0) ? 'text-bull' : 'text-bear'}`}>
                                    Day: {(Math.random() * 2 - 1).toFixed(2)}%
                                  </p>
                                </td>

                                <td className="px-6 py-5">
                                  <p className={`font-black text-base ${isUp ? 'text-bull' : 'text-bear'}`}>
                                    {isUp ? '+' : ''}{formatCurrency(Math.abs(h.gainLoss))}
                                  </p>
                                  <div className={`inline-flex items-center gap-1 text-[10px] font-black px-1.5 py-0.5 rounded mt-1 shadow-sm ${isUp ? 'bg-bull/20 text-bull border border-bull/20' : 'bg-bear/20 text-bear border border-bear/20'}`}>
                                    ...
                                    {formatPercent(Math.abs(h.gainLossPercent) / 100)}
                                  </div>
                                </td>
                                ...
                                <td className="px-6 py-5 text-right">
                                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
                                    <button onClick={(e) => e.stopPropagation()} className="px-4 py-2 bg-bull/10 hover:bg-bull hover:text-white text-bull border border-bull/20 hover:border-bull text-xs font-black rounded-xl transition-all shadow-lg hover:shadow-bull/20">
                                      BUY
                                    </button>
                                    <button onClick={(e) => e.stopPropagation()} className="px-4 py-2 bg-bear/10 hover:bg-bear hover:text-white text-bear border border-bear/20 hover:border-bear text-xs font-black rounded-xl transition-all shadow-lg hover:shadow-bear/20">
                                      SELL
                                    </button>
                                  </div>
                                </td>
